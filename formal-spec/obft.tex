...
Upon receiving the NEWVOTE signal, which is a transaction carrying a \var{vote}, the consensus rules (fig. \ref{fig:checkpoint-consensus-rules}) determine how the node state changes as a result. If the \var{vote} has a correct signature, then it is appended to the \var{votes} sequence, and we attempt to find a winner. If the winner is found, then a new \Chkp is formed and assigned to \var{lastChkp} variable. The \var{votes} sequence is cleared. 
